...
# 2. Translation Tool with Auto-Detection
def translate_to_english(text: str) -> str:
    # Check if the text is already in English
    prompt_template = PromptTemplate.from_template(
        "Is the following text in English? Answer with 'yes' or 'no': {text}"
    )
    detection_chain = prompt_template | llm
    is_english = detection_chain.invoke({"text": text}).content.strip().lower()
...
translation_tool = Tool(
    ...
    description="""Useful for translating text to English. Use this tool to ensure the output is in English.
                    Input should be the text to translate. Check if text is non-english. Don't just answer the user's question
                    in English. Translate the user prompt to English. After that you can acknoledge the user if relevant in
                    the situation.

                    Example Input- **Je m’appelle Marie. Ma famille se compose de quatre personnes. 
                    Mon mari et moi avons deux enfants, une fille de sept ans et un garçon de trois ans**
                    
                    Your Answer- *My name is Marie. My family consists of four people. My husband and I have two children, 
                    a seven-year-old daughter and a three-year-old son. *

                    Follow-up Response: "Bonjour Marie ! Il semble que vous ayez une famille adorable."
                    Meaning: Hello Marie! It sounds like you have a lovely family. How can I assist you today?
...
system_message = SystemMessage(
    ...
    Unless the user specifies otherwise, always translate non-English text to English. 
...
if st.button("Process"):
    if user_input:
        try:
            ...
            # Step 2: Ensure the output is in English by default
            translated_output = translation_tool.run(result['output'])

            llm.callbacks = []  # Remove callback after processing.

            st.write("## Output")
            st.write(translated_output)  # Display the translated output
        except Exception as e:
